...
df1=df.groupby(['country_index','location','aged_70_older'])['total_cases'].max().reset_index()
...
df3= df3[df3['total_cases'] != 0]
df_europe['deaths_per_case'] = df_europe.apply(lambda row: row.total_deaths/row.total_cases , axis = 1) 
df3['deaths_per_case'] = df3.apply(lambda row: row.total_deaths/row.total_cases , axis = 1)
...
X_train, X_test, y_train, y_test = train_test_split(X_europe, y_europe, random_state = 0,test_size=0.1)
X_train, X_test, y_train, y_test = train_test_split(X_world, y_world, random_state = 0,test_size=0.1)
